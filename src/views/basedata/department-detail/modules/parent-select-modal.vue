<script setup lang="ts">
import { computed, ref, watch } from 'vue';
import type { DataTableColumn } from 'naive-ui';
import { fetchLookupDepartment } from '@/service/api';
import { $t } from '@/locales';

defineOptions({
  name: 'ParentSelectModal'
});

interface Props {
  excludeId?: string;
}

const props = withDefaults(defineProps<Props>(), {
  excludeId: ''
});

interface Emits {
  (e: 'select', department: { id: string; name: string }): void;
}

const emit = defineEmits<Emits>();

const visible = defineModel<boolean>('visible', { default: false });

const loading = ref(false);
const allData = ref<Api.BaseData.DepartmentLookup[]>([]);
const checkedRowKeys = ref<string[]>([]);
const filterText = ref('');

const columns: DataTableColumn<Api.BaseData.Department>[] = [
  {
    type: 'selection',
    multiple: false,
    align: 'center',
    width: 48
  },
  {
    key: 'number',
    title: () => $t('page.basedata.department.number'),
    align: 'center',
    width: 120
  },
  {
    key: 'name',
    title: () => $t('page.basedata.department.name'),
    align: 'center',
    width: 150
  },
  {
    key: 'fullName',
    title: () => $t('page.basedata.department.fullName'),
    align: 'center',
    minWidth: 200
  }
];

const filteredData = computed(() => {
  let list = allData.value;

  if (props.excludeId) {
    list = list.filter(item => item.id !== props.excludeId);
  }
  if (filterText.value) {
    const keyword = filterText.value.toLowerCase();
    list = list.filter(
      item => item.number?.toLowerCase().includes(keyword) || item.name?.toLowerCase().includes(keyword)
    );
  }

  return list;
});

async function fetchData() {
  loading.value = true;
  const { error, data } = await fetchLookupDepartment();
  if (!error && data) {
    allData.value = data;
  }
  loading.value = false;
}

function getRowProps(row: Api.BaseData.DepartmentLookup) {
  return {
    style: 'cursor: pointer;',
    onclick: () => {
      if (!checkedRowKeys.value.includes(row.id)) {
        checkedRowKeys.value = [row.id];
      }
    },
    ondblclick: () => {
      emit('select', { id: row.id, name: row.name });
      visible.value = false;
    }
  };
}

function handleConfirm() {
  const selectedId = checkedRowKeys.value[0];
  if (!selectedId) {
    window.$message?.warning($t('common.selectRequired'));
    return;
  }
  const selectedRow = allData.value.find(item => item.id === selectedId);
  if (selectedRow) {
    emit('select', { id: selectedRow.id, name: selectedRow.name });
    visible.value = false;
  }
}

function handleCancel() {
  visible.value = false;
}

/** 弹窗打开时重置状态并加载数据 */
watch(visible, val => {
  if (val) {
    filterText.value = '';
    checkedRowKeys.value = [];
    fetchData();
  }
});
</script>

<template>
  <NModal
    v-model:show="visible"
    :title="$t('page.basedata.department.title')"
    preset="card"
    class="w-700px"
    :mask-closable="false"
  >
    <div class="mb-12px flex items-center gap-8px">
      <NInput v-model:value="filterText" clearable :placeholder="$t('common.keywordSearch')" class="flex-1">
        <template #prefix>
          <SvgIcon icon="ic:round-search" class="text-icon" />
        </template>
      </NInput>
    </div>

    <NDataTable
      v-model:checked-row-keys="checkedRowKeys"
      size="small"
      :columns="columns"
      :data="filteredData"
      :loading="loading"
      :row-key="row => row.id"
      :row-props="getRowProps"
      :min-height="200"
      :max-height="400"
      :scroll-x="520"
    />
    <!-- 
      @update:checked-row-keys="handleCheckChange" 
      @update:sorter="handleSorterChange"
      @update:filters="handleFiltersChange"
      @update:page="handlePageChange"
-->

    <template #footer>
      <NSpace justify="end" :size="16">
        <NButton @click="handleCancel">
          {{ $t('common.cancel') }}
        </NButton>
        <NButton type="primary" @click="handleConfirm">
          {{ $t('common.confirm') }}
        </NButton>
      </NSpace>
    </template>
  </NModal>
</template>

<style scoped></style>
